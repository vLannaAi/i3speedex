/**
 * List Producers Handler
 * GET /api/producers
 *
 * List all producers with pagination and search
 */
...
export async function handler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  ...
  try {
    ...
    // 5. Scan ALL producers (for accurate total count)
    const allItems = await scanAllItems<Producer>({
