/**
 * Unit tests for sync-producers handler
 */

import { handler } from '../../../src/handlers/sync/sync-producers';
...
describe('Sync Producers Handler', () => {
  ...
  it('should return all non-deleted producers for initial sync (no since param)', async () => {
    // Arrange
  ...
  it('should return only updated records for delta sync (with since param)', async () => {
    // Arrange
    const updatedProducer = { ...mockProducer, updatedAt: '2026-02-01T00:00:00.000Z' };
    mockScanAllItems.mockResolvedValueOnce([updatedProducer]);
  ...
  it('should include soft-deleted records in delta sync', async () => {
    ...
    mockScanAllItems.mockResolvedValueOnce([deletedProducer]);
  ...
  it('should not filter by createdBy (all users see all producers)', async () => {
    // Arrange
    mockScanAllItems.mockResolvedValueOnce([mockProducer]);
